...
public class UserStub extends Stub {
    ...
    public static class CreateValidUser extends Stub {
        public static RegisterUser.Command createUserCommandWithValidData() {
            return new RegisterUser.Command(DEFAULT_ID, faker.name().username(), faker.internet().emailAddress(), VALID_PASSWORD);
        }
    }

    public static class CreateInvalidUser extends Stub {
        public static RegisterUser.Command createUserWithShortPassword() {
            return new RegisterUser.Command(DEFAULT_ID, faker.name().username(), faker.internet().emailAddress(), faker.internet().password(3, 6));
        }

        public static RegisterUser.Command createUserWithTooLongPassword() {
            return new RegisterUser.Command(DEFAULT_ID, faker.name().username(), faker.internet().emailAddress(), faker.internet().password(55, 55));
        }

        public static RegisterUser.Command createUserWithInvalidEmail() {
            return new RegisterUser.Command(DEFAULT_ID, faker.name().username(), "invalid_email", VALID_PASSWORD);
        }
    }
}
